...
response = client.images.generate(
    model="sdxl-turbo",
    prompt='photorealistic, female eleven, fur-lined hoodie, full body, Stranger Things, snood-mouth-covering-shawl, horror describing the caption with oil and ash flaking around it large letters: "Hollow Inside" smaller: "By Byakuran"',
    n=5,
    number_steps=4,
    priority=1
)
...
response = client.vision.generate("Llava_4bit", messages=messages, image_path=http_image, max_tokens=400, priority=0)
print(response)


# Transcript
response = client.audio.transcriptions.create(model="large", file_path="ECDLR.m4a", initial_prompt="fox", language="es", priority=1)
print(response)

# Translate
response = client.audio.translations.create(model="large", file_path="ECDLR.m4a", initial_prompt="fox", language="es", priority=1)
print(response)
